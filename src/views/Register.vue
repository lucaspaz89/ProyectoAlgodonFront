  ...
  <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
      ...
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form autocomplete="off" @submit.prevent="CrearUsuario()" class="space-y-4 md:space-y-6" action="#">
                  <div>
                  ...
                  <div>
                      ...
                      <input v-model="userPassword" type="password" name="password" id="password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="">
                  </div>
                  <div>
                  ...
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      ¿Ya tienes una cuenta?  <router-link :to="{ name: 'Login' }" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Inicia sesión acá</router-link>
                  </p>                  
              </form>
          </div>
      </div>
...
export default{
  ...
  methods:{
      CrearUsuario(){
            ...
            axios.post('http://localhost:8000/api/User/registrar', jsonDatos).then(res =>{
                console.log(res)
                if(res.status === 200){
